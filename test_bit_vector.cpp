...
TEST(bit_vector, bit_vector) {
...

  {
    ...
    for (size_t i = 0; i < v.size(); ++i) { bvb.set(i, v[i]); }
    bvb.push_back(0);
    v.push_back(0);
    bvb.push_back(1);
    v.push_back(1);
  ...
  uint64_t ints[] = {uint64_t(-1), uint64_t(1) << 63, 1,           1, 1, 3, 5, 7, 0xFFF, 0xF0F, 1, 0xFFFFFF,
                     0x123456,     uint64_t(1) << 63, uint64_t(-1)};
  {
    succinct::bit_vector_builder bvb;
    for (auto i : ints) {
    ...
    for (auto i : ints) {
      uint64_t len = succinct::broadword::msb(i) + 1;
      ASSERT_EQ(i, bitmap.get_bits(pos, len));
      pos += len;
    }
  }

  {
    ...
    for (auto i : ints) { positions.push_back(positions.back() + msb(i) + 1); }
    ...
    for (size_t i = 0; i < positions.size() - 1; ++i) {
      uint64_t v   = ints[i];
      uint64_t len = positions[i + 1] - positions[i];
      ASSERT_EQ(v, bitmap.get_bits(positions[i], len));
    }
  }
}

TEST(bit_vector, bit_vector_enumerator) {
  srand(42);
  std::vector<bool> v = random_bit_vector();
...
TEST(bit_vector, bit_vector_unary_enumerator) {
  srand(42);
  uint64_t n          = 20000;
