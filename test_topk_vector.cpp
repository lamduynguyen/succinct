...
void test_topk(std::vector<value_type> const &v, TopKVector const &topkv) {
  ASSERT_EQ(v.size(), topkv.size());
  ...
  for (size_t i = 0; i < pairs_sample.size(); ++i) {
    ...
    for (uint64_t i = a; i <= b; ++i) { expected.push_back(entry_type(v[i], i)); }
