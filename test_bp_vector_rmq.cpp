...
void test_rmq(std::vector<char> const &v, BPVector const &bitmap) {
  ...
  for (size_t t = 0; t < 10; ++t) { tests.push_back(size_t(rand()) % v.size()); }
  ...
  for (size_t t = 0; t < tests.size(); ++t) {
    ...
    ASSERT_EQ(min_idx, bitmap.excess_rmq(a, a, found_min_exc));
    ...
    for (uint64_t b = a + 1; b < v.size(); ++b) {
      ...
      ASSERT_EQ(min_idx, bitmap.excess_rmq(a, b, found_min_exc));
    }
  }
}

TEST(bp_vector_rmq, test) {
  srand(42);
