...
void test_rmq(std::vector<value_type> const &v, succinct::cartesian_tree const &tree, Comparator const &comp) {
...
TEST(test_cartesian_tree, basic) {
...

  {
    ...
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); ++i) {
      ...
      for (size_t i = 0; i < v.size(); ++i) { v[i] = size_t(rand()) % 1024; }
